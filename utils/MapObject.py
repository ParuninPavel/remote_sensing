...
class MapObject (object):
    ...
    def get_class_patches(self, cls, quantity, ratio=0.5, patch_size = (256,256), scale = 1, aug = True, random_shear = True):
        # Initiallizing initial parameters
        centroids = []
        img_patches = []
        mask_patches = []
        ...
        # Quantities of random and class patches calculation
        if quantity == -1:
            class_quantity = len(centroids)
            random_quantity = round(class_quantity*(1-ratio)/ratio)
        else:
            class_quantity = round(quantity*ratio)
            random_quantity = quantity - class_quantity
        
        # Check number of centroids
        if len(centroids) == 0:
            random_quantity = 0 
        elif len(centroids) < class_quantity:
        ...
        # Creating list of coordinates consisting of centroids and random coordinates
        random.shuffle(centroids)
        coords = centroids[:class_quantity]
        for i in range (random_quantity):
            coord_h = random.randint(0, h-size[0])
            coord_w = random.randint(0, w-size[1])
            coords.append((coord_h, coord_w))
    ...
    def _augmentation(self, img, mask = None, rotation = True, mirror = True, zoom = False):
        img_out = img
        mask_out = mask
        # Mirror augmentation
        if mirror == True:
